...
def conectar_planilha():
    ...
    # Tratamento automático para quebras de linha na chave privada do Google
    ...
    # Tenta abrir a aba "Lancamentos", se não existir, usa a primeira aba ativa
    try:
...
try:
    ...
    conexao_ok = False
    st.error(f"Erro ao conectar com a planilha. Verifique as credenciais nos Secrets. Detalhe: {e}")
...
# TRATAMENTO DE DATAS ROBUSTO (Para evitar quebras com dados antigos do Google Forms)
def converter_para_data(valor_data):
    try:
        # Tenta formato padrão do app (AAAA-MM-DD)
        return datetime.strptime(str(valor_data).split()[0], "%Y-%m-%d").date()
    except ValueError:
        try:
            # Tenta formato do Google Forms brasileiro (DD/MM/AAAA)
        ...
        with st.form("form_lancamento", clear_on_submit=True):
            ...
            if botao_salvar:
                if descricao and valor > 0:
                    ...
                    # Adiciona nova linha na planilha do Google
...
if conexao_ok:
    ...
    if not dados_brutos.empty:
        # Normalização de colunas caso o usuário use a mesma planilha antiga
        colunas_necessarias = {
            'Data': 'Data', 'Descricao': 'Descricao', 'Valor': 'Valor', 
            'Categoria': 'Categoria', 'Tipo': 'Tipo', 'Responsavel': 'Responsavel', 
            'Forma_Pagamento': 'Forma_Pagamento', 'Parcelado': 'Parcelado', 'Parcelas_Totais': 'Parcelas_Totais'
        }
        
        # Mapeamento para garantir compatibilidade se a planilha tiver nomes ligeiramente diferentes
        for col_esperada in colunas_necessarias.keys():
            if col_esperada not in dados_brutos.columns:
                # Cria colunas padrão se não existirem (evita que o app quebre com dados antigos)
                if col_esperada == 'Parcelado': dados_brutos['Parcelado'] = 'Não'
                elif col_esperada == 'Parcelas_Totais': dados_brutos['Parcelas_Totais'] = 1
                elif col_esperada == 'Forma_Pagamento': dados_brutos['Forma_Pagamento'] = 'Pix'
                else: dados_brutos[col_esperada] = ""
        ...
        for index, row in dados_brutos.iterrows():
            ...
            # Sanitização e conversão de valores numéricos
            try:
            ...
            val_parcela = valor_total / total_parc if str(row['Parcelado']).strip().lower() == 'sim' else valor_total
            
            # Loop para gerar os lançamentos futuros correspondentes a cada mês de parcela
        ...
        with tabs[1]:
            ...
            # Filtro por Mês (Baseado no fechamento inteligente)
            ...
            # KPIs Principais de visualização rápida
        ...
        with tabs[2]:
            ...
            if not df_futuro.empty:
                ...
                # Cria tabela pivotada por mês e cartão para fácil leitura
    ...
    st.warning("Não foi possível carregar o painel porque a conexão com o Google Sheets falhou.")
