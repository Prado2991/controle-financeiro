import streamlit as stimport pandas as pdfrom datetime import datetime, date, timedeltafrom dateutil.relativedelta import relativedeltaimport gspreadfrom google.oauth2.service_account import Credentialsimport jsonimport plotly.express as pximport plotly.graph_objects as goimport pytzConfiguração da página para visualização Mobile-First e Temática Elegantest.set_page_config(page_title="Finanças Jonathan",page_icon="💰",layout="wide",initial_sidebar_state="collapsed")Configuração de fuso horário brasileiro para evitar que o calendário mude de dia antes da horadef obter_hoje_brasil():fuso_br = pytz.timezone("America/Sao_Paulo")agora_br = datetime.now(fuso_br)return agora_br.date()hoje_brasil = obter_hoje_brasil()Estilização CSS personalizada para visual premium, animações fluidas e cores Indigo/Emeraldst.markdown("""""", unsafe_allow_html=True)FUNÇÃO DE CONVERSÃO BRASILEIRA DE MOEDAdef formatar_brl(valor):try:val = float(valor)return f"R$ {val:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")except:return "R$ 0,00"TRATAMENTO NUMÉRICO DE ENTRADAdef tratar_entrada_numerica(texto_valor):if not texto_valor:return 0.0try:texto_limpo = str(texto_valor).replace("R$", "").replace("r$", "").strip()if "," in texto_limpo and "." in texto_limpo:texto_limpo = texto_limpo.replace(".", "").replace(",", ".")elif "," in texto_limpo:texto_limpo = texto_limpo.replace(",", ".")val_float = float(texto_limpo)return round(val_float, 2)except Exception as e:return 0.0CONEXÃO COM O GOOGLE SHEETS COM DIAGNÓSTICO DETALHADOdef conectar_planilha():scope = ["https://www.googleapis.com/auth/spreadsheets","https://www.googleapis.com/auth/drive"]if "google_credentials" not in st.secrets:
...
try:
    ...
    return None
sheet_conn = conectar_planilha()LÓGICA DE COBRANÇA DA FATURA (REGRA SOLICITADA POR JONATHAN)Compra em Cartão:Dia 01 até Dia 06 -> Pertence à fatura do MÊS ANTERIOR.Dia 07 em diante -> Pertence à fatura do MÊS ATUAL.Outras formas (Pix, Dinheiro, etc): Mês real do calendário.def calcular_mes_competencia(data_compra, forma_pagamento):if "Cartão" not in forma_pagamento:return data_compra.strftime("%Y-%m")dia = data_compra.day
if dia < 7:
...
INTERFACE DO USUÁRIOst.markdown('💰 Controle Financeiro Familiar', unsafe_allow_html=True)st.markdown("### Jonathan Prado")Lógica de contagem de fechamento baseada na data de SPvencimento_limite = date(hoje_brasil.year, hoje_brasil.month, 7)if hoje_brasil.day >= 7:vencimento_limite = vencimento_limite + relativedelta(months=1)dias_restantes = (vencimento_limite - hoje_brasil).daysst.info(f"⏳ Fechamento de Faturas: Faltam {dias_restantes} dias para o fechamento dos cartões (Fechamento em 06/{vencimento_limite.strftime('%m/%Y')} às 23:59)")tabs = st.tabs(["📲 Novo Lançamento", "📊 Dashboard & Resumos", "💳 Controle de Parcelas & Assinaturas", "✏️ Ajustar Lançamentos"])TAB 1: FORMULÁRIO DE LANÇAMENTO (OTIMIZADO PARA BRASIL)with tabs[0]:st.subheader("Registrar Gasto ou Entrada")if sheet_conn is None:st.info("⚠️ O formulário de envio está temporariamente desativado devido a problemas de conexão.")else:with st.form("form_lancamento", clear_on_submit=True):col1, col2 = st.columns([1, 1])with col1:# Calendário iniciando na data real do fuso de SPdata = st.date_input("Data do Lançamento", hoje_brasil, format="DD/MM/YYYY")descricao = st.text_input("Descrição", placeholder="Ex: Sorveteria Sávio, Roupas na Shein, Mercado Muffato")            valor_texto = st.text_input("Valor (R$)", value="0,00", help="Use vírgula para centavos. Exemplo: 8,99 ou 150,50")
...
INTERPRETAÇÃO E PROJEÇÃO DOS DADOSif sheet_conn is not None:try:dados_brutos = pd.DataFrame(sheet_conn.get_all_records())except Exception as e:dados_brutos = pd.DataFrame()st.warning("Aguardando lançamentos na aba 'Lancamentos' para carregar os gráficos.")if not dados_brutos.empty:
    ...
    for index, row in dados_brutos.iterrows():
        ...
        if tipo_lanc == "Assinatura":
            for m in range(12):
                ...
                # APLICANDO A NOVA REGRA RETROATIVA E ATUAL
...
else:st.info("Aguardando os primeiros dados da planilha para renderizar os gráficos.")
